...
class Util {
    ...
    static final int MONTH_REMINDER = 3;
    static final int WEEK_REMINDER = 2;
    static final int DAY_REMINDER = 1;
    ...
    //creates an id for the alarmmanager returns -1 if incorrect
    static int getAlarmId(int id, Boolean month, Boolean week, Boolean day) {
        if (month)
            return ConvertStringToInt(ConvertIntToString(id) + MONTH_REMINDER);
        else if (week)
            return ConvertStringToInt(ConvertIntToString(id) + WEEK_REMINDER);
        else if (day)
            return ConvertStringToInt(ConvertIntToString(id) + DAY_REMINDER);
        else
            return -1;
    }
